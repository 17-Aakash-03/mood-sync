...
# --- CONFIGURATION ---
API_KEY = "i1F8jJ9JmRqQzg-DdMdYX_RgEREafkgu"
API_SECRET = "FmH3t3gHk8ipDWQGYtnBK2jWF0RNnZrz"
...
def determine_priority_mood(emotions_dict):
    """
    NEW: Priority Threshold Logic.
    Instead of multiplying scores, we use "If ANY trace, then WIN".
    """
    # 1. Debugging: Print scores to terminal so you can see what the AI sees
    print("----- RAW SCORES -----")
    for k, v in emotions_dict.items():
        print(f"{k}: {v:.2f}")
    print("----------------------")

    # 2. PRIORITY OVERRIDES (The "If-Any" Rule)
    # If Disgust is even slightly visible (> 2%), pick it immediately.
    if emotions_dict.get('disgust', 0) > 2.0:
        return 'disgust'
    
    # If Fear is visible (> 5%), pick it.
    if emotions_dict.get('fear', 0) > 5.0:
        return 'fear'

    # If Angry is visible (> 10%), pick it.
    if emotions_dict.get('angry', 0) > 10.0:
        return 'angry'
        
    # If Sad is visible (> 15%), pick it.
    if emotions_dict.get('sad', 0) > 15.0:
        return 'sad'

    # 3. Default: Just pick the highest one (usually Happy or Neutral)
...
def analyze_image(image_file):
    ...
    if not dom:
        st.warning("☁️ Cloud API unreachable. Switching to Local AI...")
        try:
...
def main():
    ...
    if img_file is not None:
        ...
        # DEBUG: Un-comment this line if you want to see the detected mood text on screen
        # st.caption(f"Debug: Detected Mood was {mood}")
